...
class AgeSelect(discord.ui.Select):
    ...
    def __init__(self):
        ...
        super().__init__(placeholder="Wybierz swój przedział wiekowy",
                         options=options,
                         min_values=1,
                         max_values=1)

    async def callback(self, interaction: discord.Interaction):
        # Usuń inne role wiekowe (jeśli już ma jakąś)
        ...
        # Dodaj nową rolę
        selected_role = discord.utils.get(interaction.guild.roles,
                                          name=self.values[0])
        if selected_role:
            await interaction.user.add_roles(selected_role)
            await interaction.response.send_message(
                f"Nadano Ci rolę: **{selected_role.name}**", ephemeral=True)
        else:
            await interaction.response.send_message(
                "❌ Nie udało się nadać roli.", ephemeral=True)
...
class GenderSelect(Select):
    ...
    def __init__(self):
        options = [
            discord.SelectOption(label="Mężczyzna",
                                 value="Mężczyzna",
                                 emoji="👨"),
            discord.SelectOption(label="Kobieta", value="Kobieta", emoji="👩")
        ]
        super().__init__(placeholder="Wybierz swoją płeć",
                         min_values=1,
                         max_values=1,
                         options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        roles_to_remove = [
            discord.utils.get(guild.roles, name=r) for r in gender_roles
            if r != selected_role_name
        ]
        await user.remove_roles(*filter(None, roles_to_remove))
        if selected_role:
            await user.add_roles(selected_role)
        await interaction.response.send_message(
            f"✅ Nadano Ci rolę **{selected_role_name}**.", ephemeral=True)
...
async def on_ready():
    ...
    messages_to_send = [{
        "id_text": "🎯 Wybierz swój przedział wiekowy",
        "content": "**🎯 Wybierz swój przedział wiekowy z menu poniżej:**",
        "view": AgeSelectView()
    }, {
        "id_text": "🚻 Wybierz swoją płeć",
    ...
    for m in messages_to_send:
        already_sent = any(
            msg.author == bot.user and m["id_text"] in (msg.content or "")
            for msg in messages)
        if not already_sent:
...
async def on_member_join(member):
    ...
    if channel:
        embed = Embed(
            ...
            description=
            f"{member.mention}, cieszymy się, że dołączyłeś/aś do **{member.guild.name}**! 🎉",
            color=0xFFFF00)
...
async def commands_command(ctx):
    ...
    embed.add_field(name="`!commands`",
                    value="🧲 Wyświetla tę listę komend.",
                    inline=False)
    embed.add_field(name="`!ban @user [powód]`[ADM]",
                    value="🔨 Banuje użytkownika (wymaga uprawnień).",
                    inline=False)
    embed.add_field(
        name="`!mute @user`[ADM]",
        value="🔇 Mutuje użytkownika na 15 minut (wymaga uprawnień).",
        inline=False)
    embed.add_field(
        name="`!clear [liczba]`[ADM]",
        value="✅ Czyści określoną liczbę wiadomości (domyślnie 25).",
        inline=False)
    embed.add_field(name="`!profil`",
                    value="🧲 Pokazuje Twój profil: role, data dołączenia, ID.",
                    inline=False)
    embed.add_field(
        name="`!avatar [@user]`",
        value="🖼️ Wyświetla avatar Twój lub oznaczonego użytkownika.",
        inline=False)
    embed.add_field(name="`!serverinfo`",
                    value="ℹ️ Informacje o serwerze.",
                    inline=False)
    embed.add_field(name="`!rank`",
                    value="🏆 Sprawdź swój aktualny poziom i exp.",
                    inline=False)
    embed.add_field(name="`!ranking`",
                    value="🥇 Wyświetla TOP 10 użytkowników.",
                    inline=False)
...
async def ban(ctx, member: discord.Member, *, reason="Brak powodu"):
    ...
    await ctx.send(
        f"🔨 Użytkownik {member.mention} został zbanowany. Powód: {reason}")
...
async def profile(ctx):
    ...
    embed = discord.Embed(title=f"📋 Profil: {member.display_name}",
                          color=discord.Color.blue())
    embed.set_thumbnail(
        url=member.avatar.url if member.avatar else member.default_avatar.url)
    embed.add_field(name="🆔 ID", value=member.id, inline=False)
    embed.add_field(name="📆 Dołączył",
                    value=member.joined_at.strftime('%d.%m.%Y'),
                    inline=False)
    embed.add_field(name="🎭 Role",
                    value=", ".join(roles) if roles else "Brak",
                    inline=False)
    await ctx.send(embed=embed)
...
async def serverinfo(ctx):
    guild = ctx.guild
    embed = discord.Embed(title="ℹ️ Informacje o serwerze",
                          color=discord.Color.green())
    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
    embed.add_field(name="💼 Nazwa", value=guild.name, inline=False)
    embed.add_field(name="👥 Liczba członków",
                    value=guild.member_count,
                    inline=False)
    embed.add_field(name="📆 Utworzony",
...
async def avatar(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(
        title=f"🖼️ Avatar użytkownika: {member.display_name}",
